...
- **举报管理**：支持选择举报类型与补充说明；管理员处理前举报人可**补充证据图片**或**撤回举报**，撤回后可重新举报；处理中/已完成的举报不可撤回
- **后台管理**：管理员可审核、通过、拒绝、删除留言，并查看举报证据、处理举报
- **响应式布局**：适配手机和电脑端
...
若数据库中已存在旧版 `reports` 表，执行证据表迁移脚本即可（新安装会自动建表，无需执行）：

```sql
SOURCE database/migration_add_report_evidence.sql;
```

并确保 `uploads/evidence/` 目录存在且有写入权限。
